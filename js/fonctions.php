	...
	for($j=0;$j<count($proj);$j++){
		...
		";
			...
			if (($nb_user > 0) or ($nb_role > 0)) {
				...
				$sc = 0;
				while ( $tab_user = mysql_fetch_array ( $db_query ) ) {
					$aTab[$sc] = strtolower($tab_user [0]);
					$sc ++;
				}
				while ( $tab_role = mysql_fetch_array ( $query_role ) ) {
					$aTab[$sc] = strtolower($tab_role [0]);
					$sc ++;
				}
			}
...
function liste_group_user_bu_proj($group,$proj){
	...
	if(isset($proj[0] )&& $proj[0]!=""){
		...
		foreach ($aListGroup as $vG){
			if( in_array(strtolower($vG), $aListProj)){
				$aTabFinal[] = $vG;
			}
		}
...
function list_userbase_fu($proj, $cat, $user_id = "",$page = 1,$liste1, $liste2, $liste3, $gb ="") {
 ...
 * @return Integer
 */
function nbpagefu($proj, $cat, $user_id = "", $liste1, $liste2, $liste3, $gb) {
